...
static SSUavData* ssNextUav(uint8_t* mac) {
    for (int i = 0; i < SS_MAX_UAVS; i++) {
        if (memcmp(ssUavs[i].mac, mac, 6) == 0) return &ssUavs[i];
    }
    for (int i = 0; i < SS_MAX_UAVS; i++) {
        if (ssUavs[i].mac[0] == 0) return &ssUavs[i];
    }
    return &ssUavs[0];
}
...
static void ssSendJSON(const SSUavData* uav) {
    ...
    Serial.printf("{\"module\":\"skyspy\",\"mac\":\"%s\",\"rssi\":%d,"
        ...
        macStr, uav->rssi, uav->latD, uav->longD, uav->altitudeMsl,
        uav->baseLatD, uav->baseLongD, uav->uavId);
}
...
void skyspyWiFiCallback(void* buffer, wifi_promiscuous_pkt_type_t type) {
    if (!ssEnabled || type != WIFI_PKT_MGMT) return;

    wifi_promiscuous_pkt_t* packet = (wifi_promiscuous_pkt_t*)buffer;
    uint8_t* payload = packet->payload;
    int length = packet->rx_ctrl.sig_len;
    if (length > 4) length -= 4; // Strip FCS (not present in payload buffer)
    ...
    if (memcmp(nan_dest, &payload[4], 6) == 0) {
        ...
        if (ssMutex && xSemaphoreTake(ssMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
            ...
            if (odid_wifi_receive_message_pack_nan_action_frame(&ssUasData, nan_mac, payload, length) == 0) {
                SSUavData uav;
...
void SkySpyModule::onBLEAdvertisement(NimBLEAdvertisedDevice* device) {
    ...
    if (!ssMutex || xSemaphoreTake(ssMutex, pdMS_TO_TICKS(50)) != pdTRUE) return;
...
void SkySpyModule::registerRoutes(AsyncWebServer& server) {
    server.on("/api/skyspy/drones", HTTP_GET, [](AsyncWebServerRequest* r) {
        ...
        if (ssMutex && xSemaphoreTake(ssMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
            bool first = true;
            for (int i = 0; i < SS_MAX_UAVS; i++) {
                if (ssUavs[i].mac[0] == 0) continue;
                if (!first) json += ",";
                first = false;
                char macStr[18];
                snprintf(macStr, sizeof(macStr), "%02x:%02x:%02x:%02x:%02x:%02x",
                    ssUavs[i].mac[0], ssUavs[i].mac[1], ssUavs[i].mac[2],
                    ssUavs[i].mac[3], ssUavs[i].mac[4], ssUavs[i].mac[5]);
                json += "{\"mac\":\""; json += macStr;
                json += "\",\"rssi\":"; json += String(ssUavs[i].rssi);
                json += ",\"drone_lat\":"; json += String(ssUavs[i].latD, 6);
                json += ",\"drone_long\":"; json += String(ssUavs[i].longD, 6);
                json += ",\"altitude\":"; json += String(ssUavs[i].altitudeMsl);
                json += ",\"height\":"; json += String(ssUavs[i].heightAgl);
                json += ",\"speed\":"; json += String(ssUavs[i].speed);
                json += ",\"heading\":"; json += String(ssUavs[i].heading);
                json += ",\"pilot_lat\":"; json += String(ssUavs[i].baseLatD, 6);
                json += ",\"pilot_long\":"; json += String(ssUavs[i].baseLongD, 6);
                json += ",\"uav_id\":\""; json += ssUavs[i].uavId;
                json += "\",\"op_id\":\""; json += ssUavs[i].opId;
                json += "\",\"last_seen\":"; json += String(ssUavs[i].lastSeen);
                json += "}";
            }
            xSemaphoreGive(ssMutex);
        }
    ...
    server.on("/api/skyspy/status", HTTP_GET, [](AsyncWebServerRequest* r) {
        ...
        if (ssMutex && xSemaphoreTake(ssMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
            for (int i = 0; i < SS_MAX_UAVS; i++) {
                if (ssUavs[i].mac[0] != 0 && (now - ssUavs[i].lastSeen) < 7000) count++;
            }
            xSemaphoreGive(ssMutex);
        }
        char buf[100];
        snprintf(buf, sizeof(buf), "{\"active_drones\":%d,\"in_range\":%s}",
                 count, ssDeviceInRange ? "true" : "false");
...
bool SkySpyModule::isEnabled() { return _enabled; }
