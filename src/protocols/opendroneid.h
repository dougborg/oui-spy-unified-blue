...
typedef enum ODID_authtype {
    ...
    ODID_AUTH_SPECIFIC_AUTHENTICATION =
        5, // Specific auth method. The exact authentication type is indicated by the
...
typedef struct ODID_BasicID_data {
    ...
    char UASID[ODID_ID_SIZE + 1]; // Additional byte to allow for null term in normative form. Fill
                                  // unused space with NULL
} ODID_BasicID_data;

typedef struct ODID_Location_data {
    ODID_status_t Status;
    float Direction; // Degrees. 0 <= x < 360. Route course based on true North. Invalid, No Value,
                     // or Unknown: 361deg
    float SpeedHorizontal; // m/s. Positive only. Invalid, No Value, or Unknown: 255m/s. If speed is
                           // >= 254.25 m/s: 254.25m/s
    ...
    float TimeStamp; // seconds after the full hour relative to UTC. Invalid, No Value, or Unknown:
                     // 0xFFFF
} ODID_Location_data;
...
typedef struct ODID_SelfID_data {
    ...
    char Desc[ODID_STR_SIZE + 1]; // Additional byte to allow for null term in normative form. Fill
                                  // unused space with NULL
} ODID_SelfID_data;
