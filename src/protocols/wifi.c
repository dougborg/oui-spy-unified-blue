...
static int buf_fill_ieee80211_mgmt(uint8_t* buf, size_t* len, size_t buf_size,
                                   const uint16_t subtype, const uint8_t* dst_addr,
                                   const uint8_t* src_addr, const uint8_t* bssid) {
...
#warning "Unable to set wifi timestamp."
    ...
    beacon->capability =
        cpu_to_le16(IEEE80211_CAPINFO_SHORT_SLOTTIME | IEEE80211_CAPINFO_SHORT_PREAMBLE);
    *len += sizeof(*beacon);
...
int odid_message_build_pack(ODID_UAS_Data* UAS_Data, void* pack, size_t buflen) {
    ...
    for (int i = 0; i < ODID_BASIC_ID_MAX_MESSAGES; i++) {
        if (UAS_Data->BasicIDValid[i]) {
            if (msg_pack.MsgPackSize >= ODID_PACK_MAX_MESSAGES)
                return -EINVAL;
            if (encodeBasicIDMessage((void*)&msg_pack.Messages[msg_pack.MsgPackSize],
                                     &UAS_Data->BasicID[i]) == ODID_SUCCESS)
    ...
    if (UAS_Data->LocationValid) {
        if (msg_pack.MsgPackSize >= ODID_PACK_MAX_MESSAGES)
            return -EINVAL;
        if (encodeLocationMessage((void*)&msg_pack.Messages[msg_pack.MsgPackSize],
                                  &UAS_Data->Location) == ODID_SUCCESS)
            msg_pack.MsgPackSize++;
    }
    for (int i = 0; i < ODID_AUTH_MAX_PAGES; i++) {
        if (UAS_Data->AuthValid[i]) {
            if (msg_pack.MsgPackSize >= ODID_PACK_MAX_MESSAGES)
                return -EINVAL;
            if (encodeAuthMessage((void*)&msg_pack.Messages[msg_pack.MsgPackSize],
                                  &UAS_Data->Auth[i]) == ODID_SUCCESS)
                msg_pack.MsgPackSize++;
        }
    }
    if (UAS_Data->SelfIDValid) {
        if (msg_pack.MsgPackSize >= ODID_PACK_MAX_MESSAGES)
            return -EINVAL;
        if (encodeSelfIDMessage((void*)&msg_pack.Messages[msg_pack.MsgPackSize],
                                &UAS_Data->SelfID) == ODID_SUCCESS)
            msg_pack.MsgPackSize++;
    }
    if (UAS_Data->SystemValid) {
        if (msg_pack.MsgPackSize >= ODID_PACK_MAX_MESSAGES)
            return -EINVAL;
        if (encodeSystemMessage((void*)&msg_pack.Messages[msg_pack.MsgPackSize],
                                &UAS_Data->System) == ODID_SUCCESS)
            msg_pack.MsgPackSize++;
    }
    if (UAS_Data->OperatorIDValid) {
        if (msg_pack.MsgPackSize >= ODID_PACK_MAX_MESSAGES)
            return -EINVAL;
        if (encodeOperatorIDMessage((void*)&msg_pack.Messages[msg_pack.MsgPackSize],
                                    &UAS_Data->OperatorID) == ODID_SUCCESS)
            msg_pack.MsgPackSize++;
    }
    ...
    len =
        sizeof(*msg_pack_enc) - (ODID_PACK_MAX_MESSAGES - msg_pack.MsgPackSize) * ODID_MESSAGE_SIZE;
...
int odid_wifi_build_nan_sync_beacon_frame(char* mac, uint8_t* buf, size_t buf_size) {
    ...
    ret = buf_fill_ieee80211_mgmt(buf, &len, buf_size, IEEE80211_STYPE_BEACON, target_addr,
                                  (uint8_t*)mac, cluster_id);
    if (ret < 0)
        return ret;
    ...
    memset(cluster_attr->anchor_master_beacon_transmission_time, 0,
           sizeof(cluster_attr->anchor_master_beacon_transmission_time));
...
int odid_wifi_build_message_pack_nan_action_frame(ODID_UAS_Data* UAS_Data, char* mac,
    ...
    /* IEEE 802.11 Management Header */
    ret = buf_fill_ieee80211_mgmt(buf, &len, buf_size, IEEE80211_STYPE_ACTION, target_addr,
                                  (uint8_t*)mac, cluster_id);
    if (ret < 0)
        return ret;
    ...
    nsda->header.length = cpu_to_le16(sizeof(*nsda) - sizeof(struct nan_attribute_header) +
                                      nsda->service_info_length);
...
int odid_message_process_pack(ODID_UAS_Data* UAS_Data, uint8_t* pack, size_t buflen) {
    ...
    size_t size = sizeof(*msg_pack_enc) -
                  ODID_MESSAGE_SIZE * (ODID_PACK_MAX_MESSAGES - msg_pack_enc->MsgPackSize);
    if (size > buflen)
        return -ENOMEM;
...
int odid_wifi_receive_message_pack_nan_action_frame(ODID_UAS_Data* UAS_Data, char* mac,
    ...
    if (nsda->service_info_length != (sizeof(*si) + ret))
        return -EINVAL;
    if (nsda->header.length != (cpu_to_le16(sizeof(*nsda) - sizeof(struct nan_attribute_header) +
                                            nsda->service_info_length)))
        return -EINVAL;
    len += sizeof(*si) + ret;
